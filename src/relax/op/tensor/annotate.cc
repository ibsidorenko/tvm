...
Expr smooth(Expr x, Expr m, Expr zp, int k, String md) {
  ...
  return Call(op, {std::move(x), std::move(m), std::move(zp)}, Attrs(attrs));
}
...
TVM_REGISTER_OP("relax.annotate.smooth")
    ...
    .add_argument("zero_point", "Tensor", "Tensor zero point.")
